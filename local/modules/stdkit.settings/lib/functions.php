<?php
	...
	if( !function_exists( '__AdmSettingsSaveOptions' ) ) {
	...
	if( !function_exists( '__AdmSettingsSaveOption' ) ) {
		function __AdmSettingsSaveOption( $module_id, $arOption )
		{
			...
			if( $isChoiceSites ) {
				if( isset( $_REQUEST[ $name . "_all" ] ) && $_REQUEST[ $name . "_all" ] <> '' )
					COption::SetOptionString( $module_id, $name, $_REQUEST[ $name . "_all" ], $arOption[1] ); else
					COption::RemoveOption( $module_id, $name );
				$queryObject = \Bitrix\Main\SiteTable::getList( array( 'select' => array( 'LID', 'NAME' ), 'filter' => array(), 'order' => array( 'SORT' => 'ASC' ), ) );
				while( $site = $queryObject->fetch() ) {
					if( isset( $_REQUEST[ $name . "_" . $site["LID"] ] ) && $_REQUEST[ $name . "_" . $site["LID"] ] <> '' && !isset( $_REQUEST[ $name . "_all" ] ) ) {
	...
	if( !function_exists( '__AdmSettingsDrawRow' ) ) {
				...
				if( $Option[0] != "" ) {
					if( $isChoiceSites ) {
						...
						if( empty( $listSiteValue ) ) {
							...
							if( $value )
								$listSiteValue = array( $Option[0] . "_all" => $value ); else
	...
	if( !function_exists( '__AdmSettingsDrawList' ) ) {
	...
	if( !function_exists( 'renderLable' ) ) {
				...
				<select onchange="changeSite(this, '<?= htmlspecialcharsbx( $Option[0] ) ?>')">
					<? foreach( $listSite as $lid => $siteName ): ?>
						<option <? if( $siteValue == $lid )
							echo "selected"; ?> value="<?= htmlspecialcharsbx( $lid ) ?>">
		...
		<? else:?>
			<td<? if( $type[0] == "multiselectbox" || $type[0] == "textarea" || $type[0] == "statictext" || $type[0] == "statichtml" )
				echo ' class="adm-detail-valign-top"' ?> width="50%"><?
					if( $type[0] == "checkbox" )
						echo "<label for='" . htmlspecialcharsbx( $Option[0] ) . "'>" . $Option[1] . "</label>"; else
						echo $Option[1];
					if( $sup_text <> '' ) {
		...
		function renderInput( $Option, $arControllerOption, $fieldName, $val )
		{
			...
			if( $type[0] == "checkbox" ):
				...
				echo ' disabled title="' . GetMessage( "MAIN_ADMIN_SET_CONTROLLER_ALT" ) . '"'; ?>
						 ...
						 value="Y"<? if( $val == "Y" )
				echo " checked"; ?><?= $disabled ?><? if( isset( $type[2] ) && $type[2] <> '' )
				echo " " . $type[2] ?>><?
			elseif( $type[0] == "text" || $type[0] == "password" ):
				...
				value="<? echo htmlspecialcharsbx( $val ) ?>" name="<?= htmlspecialcharsbx( $fieldName ) ?>"<?= $disabled ?><?= ( $type[0] == "password" || isset( $type["noautocomplete"] ) && $type["noautocomplete"] ? ' autocomplete="new-password"' : '' ) ?>><?
			elseif( $type[0] == "selectbox" ):
				$arr = $type[1];
				if( !is_array( $arr ) )
					$arr = array();
				?><select
				name="<?= htmlspecialcharsbx( $fieldName ) ?>" <? if( isset( $arControllerOption[ $Option[0] ] ) )
				echo ' disabled title="' . GetMessage( "MAIN_ADMIN_SET_CONTROLLER_ALT" ) . '"'; ?> <?= $disabled ?>><?
			...
			elseif( $type[0] == "multiselectbox" ):
				...
				foreach( $arr as $key => $v ):
					?>
					<option value="<? echo $key ?>"<? if( in_array( $key, $arr_val ) )
					echo " selected" ?>><? echo htmlspecialcharsbx( $v ) ?></option><?
				endforeach;
				?></select><?
	...
	if( !function_exists( 'getHLid' ) ) {
		function getHLid()
		{
			$hlblock = HL\HighloadBlockTable::getList( [ 'filter' => [ '=NAME' => "Stdkitrequisites" ] ] )->fetch();
			return $hlblock['ID'];
		}
	}
	
	if( !function_exists( 'getHLTB' ) ) {
		function getHLTB()
		{
			$hlblock = HL\HighloadBlockTable::getList( [ 'filter' => [ '=NAME' => "Stdkitrequisites" ] ] )->fetch();
	...
	if( !function_exists( 'getHLEL' ) ) {
		function getHLEL( $e )
		{
			$rsData = $e::getList( array( "select" => array( "*" ), "order" => array( "ID" => "ASC" ), ) );
			$arList = array();
	...
	if( !function_exists( 'get_def_fields' ) ) {
		function get_def_fields()
		{
			global $USER_FIELD_MANAGER;
			$fields = array();
			$arfields = $USER_FIELD_MANAGER->getUserFieldsWithReadyData( 'HLBLOCK_' . defIDHL(), array(), LANGUAGE_ID );
			foreach( $arfields as $UF => $field ) {
				$fields[ $UF ] = $field['EDIT_FORM_LABEL'];
			}
			return $fields;
		}
	}
