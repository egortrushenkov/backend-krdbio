<? if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die();
/** @var array $arParams */
/** @var array $arResult */
/** @global CMain $APPLICATION */
/** @global CUser $USER */
/** @global CDatabase $DB */
/** @var CBitrixComponentTemplate $this */
/** @var string $templateName */
/** @var string $templateFile */
/** @var string $templateFolder */
/** @var string $componentPath */
/** @var CBitrixComponent $component */
$this->setFrameMode(true);

$title = !empty($arParams["SECTION_TITLE"])
        ? $arParams["SECTION_TITLE"]
        : $arResult["SECTION"]["NAME"];

$btn = $arParams["BTN"];
$grid = $arParams["GRID"];

?>
<section class="container overflow-hidden pt-0">
    <?php \lib\KitTPL::subtitle("{text: '".$title."', className: 'sm:text-center mb-8 sm:mb-11 lg:mb-14 clip-path clip-path-up ease-linear duration-1000', data: 'data-anim'}");?>
    <div data-slider="services">
        <?printr(count($arResult["ITEMS"]))?>
        <div class="swiper overflow-visible" data-slider-swiper="services">
            <div class="swiper-wrapper xl:grid xl:gap-6 xl:!transform-none <?= $grid ? 'xl:grid-cols-4' : 'xl:grid-cols-3' ?>">
                <? foreach ($arResult["ITEMS"] as $arItem): ?>
                <?
                $this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_EDIT"));
                $this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
                ?>
                    <div class="swiper-slide h-auto" id="<?= $this->GetEditAreaId($arItem['ID']); ?>">
                        <?php \lib\KitTPL::cardService("{price: false, src: '".$arItem['PREVIEW_PICTURE']['SRC']."', title: '".$arItem['NAME']."', text: '".$arItem['PREVIEW_TEXT']."', btn: '".$btn."', linkBtn: '".$arItem['CODE']."'}");?>
                    </div>
                <?endforeach;?>
            </div>
        </div>
    </div>
</section>

